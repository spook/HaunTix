...
sub do_full_comp {
    ...
    my $btn = $popup->show;
    return if $btn ne "OK";
    my $dcode   = $popup->{dcode};
    my $org     = $popup->{e_org}->get();
    my $who     = $popup->{e_who}->get();
    ...
    $trn->{trnNote} = "Full Comp Code:$dcode Org:'$org' Who:'$who' Authby:'$authby' Reason:'$reason'";
    $trn->fullcomp(1);
    $trn->retally;
    if ($tixnote) {
...
sub do_testix {
    ...
    # Create a dummy sale and a dummy show
# TODO Working here... ****************

    # Create a dummy ticket
    my $tic = new htx::ticket(
        htx      => $htx,
        tixId    => 1,
        tixCode  => 1,
        tixPool  => $TIX_POOL_BOOTH,
        tixState => $TIX_STATE_SOLD,
        tixNote  => "This is a TEST ticket - it is invalid.",
        );
