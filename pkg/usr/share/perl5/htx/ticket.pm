...
# mktixno with leading zeros to pad to 10 digits
sub mktixnoz {
    my ($id, $code) = @_;
    my $t = sprintf "%d%4.4d", $id, $code;
    $t = ('0' x (10-length($t))) . $t
        if length($t) < 10;
    return $t;
}
...
sub print_ticket_T055020 {
    ...
    # Get info
    my $sale     = $this->{sale};
    my $salCost  = $sale->{salCost};
    my $salName  = $sale->{salName};
    my $show     = $this->{show};
    my $shoId    = $show->{shoId};
    my $shoClass = $show->{shoClass};
    my $trnId    = $sale->{trnId};
    ...
    $rfstmt = "Early Season runs thru October 9th"
        if $shoClass =~ m/^.ES$/;     # *** FIXME - A real hack!!!

    # Special for "Parade of Haunts"
    if (($shoClass eq 'VEP') || ($shoClass eq 'VFP')) {
        $intro   = $cfg->{parade}->{intro}            || $intro;
        $name    = $cfg->{parade}->{name}             || $name;
        $desc    = $cfg->{parade}->{desc}             || $desc;
        $slogan  = $cfg->{parade}->{slogan}           || $slogan;
        $site    = $cfg->{parade}->{site}             || $site;
        $website = $cfg->{parade}->{website}          || $website;
        $addr0   = $cfg->{parade}->{addr0}            || $addr0;
        $addr1   = $cfg->{parade}->{addr1}            || $addr1;
        $addr2   = $cfg->{parade}->{addr2}            || $addr2;
        $rftype  = $cfg->{parade}->{refund_type}      || $rftype;
        $rfstmt  = $cfg->{parade}->{refund_statement} || $rfstmt;
        $apolicy = "Valid thru Oct 6, 2013. One use each haunt. " . $this->{tixNote}
            if $shoClass eq 'VEP';
        $apolicy = "Valid for 2013 season. One use each haunt.  " . $this->{tixNote}
            if $shoClass eq 'VFP';
        $salName .= " VIP";
    }

    my $ticnum  = mktixnoz($this->{tixId}, $this->{tixCode});
    my $when1
    ...
    my $when2
        ...
        : $sale->{salIsDaily} ? UnixDate($show->{shoTime}, q{%a %d %b}) . " - $shoClass"
        :                       $salName;
    my $sp1 = q{ } x int((25-length($when1))/2);
    $when2 =~ s/^0//g;
    ###$when2 =~ s/\s0//g;
    $when2 =~ s/AM/am/;
    $when2 =~ s/PM/pm/;
    ...
    $drv->cmd("1311000" . "0514" . "0040" . "Admit One $shoClass");

    $drv->cmd("1911A12" . "0485" . "0005" . $sp1 . $when1);
    $drv->cmd("1X11" . "000" . "0460" . "0001" . "L195021");    # Black bar
    $drv->cmd("1311000" . "0463" . "0047" . $rftype);

    $drv->cmd("1111000" . "0452" . "0037" . "HaunTix $htx::HTX_VERSION TID$trnId S#$shoId");

    $drv->cmd("2911A08" . "0440" . "0177" . $intro);
    $drv->cmd("2911A24" . "0440" . "0140" . $name);
    $drv->cmd("2911A10" . "0440" . "0127" . $desc);
    $drv->cmd("2911A18" . "0440" . "0098" . $when2);
    $drv->cmd("2911A08" . "0440" . "0083" . $apolicy);  # Policy or ticket note

    $drv->cmd("2e02030" . "0440" . "0051" . $ticnum);   # Thin Barcode
    $drv->cmd("2911A08" . "0275" . "0069" . "Face Value");
    $drv->cmd("2911A12" . "0270" . "0051" . dollar($salCost));

    $drv->cmd("2911A12" . "0440" . "0030" . $website);
    $drv->cmd("2911A10" . "0440" . "0013" . $addr0);
    $drv->cmd("2911A08" . "0440" . "0001" . $rfstmt);

    $drv->cmd("1911A08" . "0177" . "0062" . "VIP - VIP - VIP")
        if $shoClass =~ m/^V/;
    $drv->cmd("1e00047" . "0122" . "0030" . $ticnum);   # Main Barcode
    $drv->cmd("1311000" . "0105" . "0055" . $ticnum);   # Main HRI
#    $drv->cmd("1X11" . "000" . "0100" . "0001" . "B188350007007");   # Rectangle: Box wid hgt thk-top/bot thk-sides

    # Stub
    $drv->cmd("1X11" . "000" . "0074" . "0001" . "L190024");         # Black Bar L wid hgt
    $drv->cmd("1911A12" . "0075" . "0005" . $sp1 . $when1);
    $drv->cmd("1e00030" . "0040" . "0025" . $ticnum);                # Stub Barcode
    $drv->cmd("1211000" . "0028" . "0070" . $ticnum);                # Stub HRI
    $drv->cmd("1X11" . "000" . "0003" . "0001" . "L195021");         # Black bar
    $drv->cmd("1311000" . "0007" . "0040" . "Admit One $shoClass");
