...
package htx;
  ...
  our $HTX_VERSION = "v0.8";
  ### TODO: This below is a mess - do it a better way!
  our $CONFIG_FILE = -r '/etc/htx/hauntix.conf'
                   ? '/etc/htx/hauntix.conf'
                   : -r ($ENV{DOCUMENT_ROOT}||q{}) . q{/../etc/htx/hauntix.conf}
                        ? $ENV{DOCUMENT_ROOT} . q{/../etc/htx/hauntix.conf}
