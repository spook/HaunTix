...
my $EXPY = {'0' => 'year'};
for my $y (2014 .. 2022) {$EXPY->{$y - 2000} = $y;}
...
#
# First page: show selection, tickets, payment
#
sub _page_buy {
    my $q        = shift;
    my $alert    = shift || q{};
    ...
    foreach my $show (@$tally) {
        ...
        my $state
            ...
            :                                    $STATE_OPEN;
        $state = $STATE_NEAR if ($state == $STATE_OPEN) 
                && (($date eq '2014-10-10') || ($date eq '2014-10-16'));    # Push dates ;-)
...
<!--   <a id="fupg"  href="?step=upg">Upgrade Tickets</a> -->
  ...
  <div id="swapInfo" style="display:none">
    <p>On-line Ticket exchanges are currently disabled.  Please email 
    Tickets<img src="/htx/img/at.bmp" border="0" width="9px"/>HauntedMines.org 
    with your ticket numbers and the new date you want,
    to request an exchange. </p>
<!--
    <p>You may exchange tickets for another day up until 36 hours before 
...
-->
  ...
  <form method="post" action="htxTix.cgi" enctype="multipart/form-data" id="htxForm">
    ...
    <div id="datepick"></div>
    <div class="legend">
      <span class="box_open">&#9633;</span> Discount Days &nbsp;
      <span class="box_near">&#9633;</span> Normal Price  &nbsp;
      <span class="box_full">&#9633;</span> Premium Night &nbsp;
    ...
    <input type="hidden" name="expect"   id="expect"   value="0"/>
      ...
      <div id="ntix" class="ui-helper-reset ui-widget-content ui-corner-all">
		<table id="ntixtable">
          ...
          <tr>
            ...
            <td width="33%">How many?&nbsp;<input id="ntixREG" name="ntixREG" value="0" size="2" maxlength="3" onkeyup="onlydigits(this);"/></td>
          ...
          <tr>
            ...
            <td width="33%">How many?&nbsp;<input id="ntixVIP" name="ntixVIP" value="0" size="2" maxlength="3" onkeyup="onlydigits(this);"/></td>
    ...
    <h2>Step 5: Agree to Conditions</h2>
      <div class="conditions">
      By checking the box below and placing this order, you agree to the following:
        <ul>
          <li>Tickets are valid ONLY for the date purchased -- as printed on the ticket.
          <li>Ticket quantities are limited and shows may SELL OUT.</li>
          <li><b>NO REFUNDS</b>.</li>
          <li><b>NO EXCHANGES</b>.</li>
          <li>You won't run with scissors.</li>
          <li>You'll be kind to monsters, especially on Halloween.</li>
          <li>You'll clean your room and thank your parents often.</li>
        </ul>
        For full details, see our <a href="/hmTix.html" target="_blank">Ticket Policy</a> page.
      </div>
      <div id="agree" class="ui-helper-reset ui-widget-content ui-corner-all">
        <input type="checkbox" id="agreecheck" onclick="tally_order();"> &nbsp; Yes, I agree to the ticket policy.
      </div>
...
^;
    ...
    foreach my $sale ($trn->sales()) {
        ...
        if ($sale->{show} && ($sale->{salIsDaily} || $sale->{salIsTimed})) {
            my $show = $sale->{show}; 
    ...
    print $q->p("Regenerating tickets...");
...
^;
    ...
    print $q->h2("Do NOT refresh this page");
    print $q->start_pre;
    ...
    my $trn = htx::transaction->new(
        ...
        -trnStation    => uc($cfg->{web}->{station} || $htx::transaction::TRN_STATIONS_WEB[0]),
    );
    ...
    my $ntix = 0;   # total number of tickets
    my $ndsc = 0;   # number of discount-eligible ticket count
    foreach my $want (@wantshows) {
        ...
        my $item = {
            ...
            itmIsTaxable => 0,
            itmIsTicket  => 1,
            itmIsDaily   => 1||$show->{shoIsDaily}, ### FIXME! prd has daily/timed, not shows!
            itmIsTimed   => $show->{shoIsTimed},    ### FIXME! ditto.
            tixPool      => $TIX_POOL_WEB,
        };
        $trn->add_item($item, $qty);
        $ntix += $qty;
        $ndsc += $qty if $show->{shoTime} gt '2014-10-09';   # *** Yikes! Hardocded HACK - FIXME !
        if ($trn->error) {
            my $t1err = $trn->error;
            $trn->cancel;
            my $t0err = $trn->error() || "Transaction $trn->{trnId} cancelled.";
    ...
    my $group_threshold = 10;                    ### TODO: Get threshold from d/b
    if ($ndsc >= $group_threshold) {
        # compute amount off
        ### TODO: get all this from d/b
        my $off 
            = $ndsc >= 30? 300*$ndsc
            : $ndsc >= 20? 200*$ndsc
            : $ndsc >= 10? 100*$ndsc
    ...
    my $sc      = $persale + int($trn->total() * $percent / 100.0 + 0.5);
    $sc = int(($sc + $roundup - 1) / $roundup) * $roundup if $roundup;
    $trn->servicecharge($sc);
    $trn->retally();

    # Compare expected cost to computed cost
    my $expect = $q->param('expect') || 0;
    if ($expect != $trn->total()) {
        $trn->cancel;
        my $expamt = dollars($expect);
        my $totamt = dollars($trn->total());
        exit _page_error($q,
             "Eeeeek!\n"
            ."The actual cost of the purchase ($totamt) does not match the expected cost ($expamt).  "
            ."This transaction  has been cancelled.  Your card has NOT been charged.");
    }
    ...
    print "  Submitting... ";
    my @namparts = split(/\s+/, $q->param('nameoncard') || q{}, 2);
    my $fnam = $namparts[0];
    my $lnam = $namparts[1];
    my %ccdat = (
        ...
        FirstName       => $fnam,
        LastName        => $lnam,
        Company         => $q->param('email'), # put email here, so the gateway doesn't do the mailing!
        URef            => "HTX TID " . $trn->trnid,
    );
    ...
    $trn->cc($crg) 
        ...
        || ($rsp->{ResponseCode} eq "000");    # add it to the transaction
    print "done\n";
    ...
    # Credit card go thru?
    if (($rsp->{ResponseCode} != 0) && ($rsp->{ResponseCode} ne "000")) {
        ...
        $trn->cancel;
        my $t0err = $trn->error() || "Haunted transaction $trn->{trnId} cancelled.";
    ...
    print $q->h2("Do NOT refresh this page -- avoid duplicate charges!");
    print $q->br;
    ...
    print $q->p("If you lose the email and the PDF available from this page, "
            ...
            . "Treat this code like cash - save it in a safe place:");
